...
const InfoCard = ({ icon, label, value, color }) => (
    ...
    >
        <div css={tw`mr-3 text-xl`} style={{ color }}>{icon}</div>
...
const Section = ({ title, icon, iconColor, data }) => (
    <div css={tw`bg-white rounded-xl border border-gray-200 p-4 mb-6`}>
        <div css={tw`flex items-center mb-4`}>
            <span css={tw`mr-2 text-xl`} style={{ color: iconColor }}>{icon}</span>
...
const AddressSection = ({ address }) => (
    <div css={tw`bg-white rounded-xl border border-gray-200 p-4`}>
        <div css={tw`flex items-center mb-4`}>
            <span css={tw`mr-2 text-xl text-yellow-600`}><HomeOutlined /></span>
...
const CustomerInfo = () => {
    ...
    const addressData = {
        title: 'test uhuh',
        detail: 'test uhuh, Huyện Mường Khương, Lào Cai, Viet Nam'
    }
